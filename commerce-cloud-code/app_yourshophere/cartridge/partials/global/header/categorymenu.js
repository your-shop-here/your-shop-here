...
function createModel(apiCategory) {
    const menuStyle = request.httpParameterMap.menuStyle.stringValue || 'large';
    const model = {
        apiCategory,
        menuStyle,
    };
    ...
    lazyload(model, 'children', () => apiCategory.onlineSubCategories.toArray(0, 50)
        .map((subCategory) => createCategoryModel(subCategory))
        .filter((cat) => !cat.hidden));
    lazyload(model, 'url', () => dw.web.URLUtils.url('Search-Show', 'cgid', apiCategory.ID));
    return model;
}

function createCategoryModel(apiCategory) {
    const model = { apiCategory };
    lazyload(model, 'hidden', () => !apiCategory.custom.yshShowInMenu);
    lazyload(model, 'children', () => apiCategory.onlineSubCategories.toArray(0, 50)
        .map((subCategory) => createCategoryModel(subCategory))
        .filter((cat) => !cat.hidden));
...
function template(model) {
    return `
        ...
        <ul id="menu-root" class="menu-style-${model.menuStyle}">
