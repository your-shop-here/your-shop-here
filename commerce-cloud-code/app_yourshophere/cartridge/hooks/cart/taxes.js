...
function calculateTaxes(basket) {
    ...
    shipments.toArray().forEach((shipment) => {
        ...
        const lineItems = shipment.getAllLineItems();

        lineItems.toArray().forEach((lineItem) => {
            ...
            // get the tax rate
            const taxRate = TaxMgr.getTaxRate(taxClassId, taxJurisdictionId);
